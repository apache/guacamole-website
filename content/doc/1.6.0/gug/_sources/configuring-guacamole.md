...
images](guacamole-docker), these same configuration aspects are largely driven
...
Standard configuration options / `guacamole.properties`
-------------------------------------------------------
...
authentication providers. If using [the Docker images](guacamole-docker), these
...
using [the Docker images](guacamole-docker), the same logs are visible in the
...
{samp}`$\{{TOKEN_NAME}:{MODIFIER}\}`. The following modifiers are
currently supported:
